...
/**
 * Full-screen "Writing your draft" loader shown while the backend turns the
 * answers + outline into a full draft. Twin of OutlineBuildLoader (portal to
 * <body>, simulated progress, rotating stages/tips) so the two long AI waits
 * feel like one product; only the copy differs. The backend work isn't streamed,
 * so progress + stages are simulated on a timer to signal steady activity.
 */

// Draft-writing stages, cycled as the simulated progress climbs.
const STAGES = [
...
export function DraftBuildLoader() {
  ...
  // shows its own content during SSR/initial load), so there's no hydration mismatch.
  ...
  return createPortal(
    ...
    >
      <style>{`
        @keyframes dbl-fade { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }
      `}</style>
      ...
      >
        ...
        {/* heading + subtitle */}
        <h2 className="mt-5 text-xl font-bold text-indigo-700">Writing your draft</h2>
        <p className="mt-1 text-xs text-slate-400">
          Sit tight while we turn your answers and outline into a full draft.
        </p>
        ...
        <p
          ...
          style={{ animation: "dbl-fade 0.4s ease-out" }}
        >
          {STAGES[stageIdx]}
        </p>
        ...
        <div className="mt-2 flex items-center justify-center gap-2 font-mono text-xs text-indigo-500">
          ...
          <span key={`word-${wordIdx}`} style={{ animation: "dbl-fade 0.4s ease-out" }}>
