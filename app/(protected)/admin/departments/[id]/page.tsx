        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
              dept.initial_prompt && dept.system_prompt
                ? "bg-purple-100 text-purple-700"
                : "bg-amber-100 text-amber-700"
            }`}>
              {dept.initial_prompt && dept.system_prompt ? "Configured" : "Incomplete"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            {!editingPrompts && (
              <Button
                variant="outline"
                size="sm"
                onClick={(e) => { e.stopPropagation(); handleEditPrompts(); setPromptsOpen(true) }}
              >
                <Edit3 className="h-3.5 w-3.5 mr-1" /> Edit Prompts
              </Button>
            )}
            {promptsOpen ? (
          ...
          <div className="border-t px-6 py-5 space-y-5">
            {!editingPrompts ? (
              <>
                <div className="space-y-2">
                  <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                    Initial Prompt <span className="text-xs font-normal normal-case">(Agent Introduction / Context)</span>
                  </Label>
                  {dept.initial_prompt ? (
                    <div className="rounded-lg bg-muted/40 p-4 text-sm leading-relaxed whitespace-pre-wrap font-mono text-xs">
                      {dept.initial_prompt}
                    </div>
                  ) : (
                    <div className="rounded-lg border border-dashed border-amber-300 bg-amber-50 p-4">
                      <p className="text-sm text-amber-700 flex items-center gap-2">
                        ...
                        No initial prompt set. Click &quot;Edit Prompts&quot; to configure the AI agent introduction.
                      </p>
                    </div>
                  )}
                </div>
                <div className="space-y-2">
                  <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                    System Prompt <span className="text-xs font-normal normal-case">(Core Instructions / Behavioral Rules)</span>
                  </Label>
                  {dept.system_prompt ? (
                    <div className="rounded-lg bg-muted/40 p-4 text-sm leading-relaxed whitespace-pre-wrap font-mono text-xs">
                      {dept.system_prompt}
                    ...
                    <div className="rounded-lg border border-dashed border-amber-300 bg-amber-50 p-4">
                      <p className="text-sm text-amber-700 flex items-center gap-2">
                        <Info className="h-4 w-4 shrink-0" />
                        No system prompt set. Click &quot;Edit Prompts&quot; to configure AI behavioral rules.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <>
                <div className="space-y-2">
                  <Label>
                    Initial Prompt <span className="text-destructive">*</span>
                    <span className="text-xs text-muted-foreground font-normal ml-1">(Agent Introduction / Context)</span>
                  </Label>
                  <Textarea
                    value={initialPrompt}
                    onChange={(e) => setInitialPrompt(e.target.value)}
                    placeholder="e.g. You are the AI assistant for the Finance Department..."
                    rows={5}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="e.g. Always respond in a professional, executive-level tone..."
                    rows={5}
                    className="text-sm font-mono"
                  />
                </div>
                <div className="flex justify-end gap-3">
                  <Button variant="outline" onClick={() => setEditingPrompts(false)}>
                    <X className="h-3.5 w-3.5 mr-1" /> Cancel
                  </Button>
                  <Button
                    onClick={handleSavePrompts}
                    disabled={updateMutation.isPending || !initialPrompt.trim() || !systemPrompt.trim()}
                  >
                    <Save className="h-3.5 w-3.5 mr-1" />
